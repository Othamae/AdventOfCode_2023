...
function getDesintegratedBricks(bricks: Brick[]) {
    ...
    return calculateFallingBricks(bricks)
}
...
function calculateFallingBricks(bricks: Brick[]) {
    let result = 0
    for (let i = 0;i < bricks.length;i++) {
        const bricksClone = JSON.parse(JSON.stringify(bricks))
        const queue = [i]
        while (queue.length > 0) {
            const currentBrick = queue.shift()
            for (let j = 0;j < bricksClone.length;j++) {
                if (currentBrick === j) continue
                const brick = bricksClone[j]
                if (brick.supportedBy.includes(currentBrick)) {
                    brick.supportedBy.splice(brick.supportedBy.indexOf(currentBrick), 1)
